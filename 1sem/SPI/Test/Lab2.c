...
void truncation (int result_rank, int result_transfer[], int result_truncation[]){
  ...
  for (int i = 32 - result_rank, y = 0; i < 31; i++, y++) {
...
void f1 (int result_rank, int result_truncation[]) {
  ...
  f0(result_rank);
  for (int i = result_rank + 1, y = 0; i < 2 * result_rank; i++, y++) {
...
void f2(int result_rank_old, int result_truncation_old[]){
  ...
  for (int i = 0; i < result_rank_old - 1; i++) {
...
int main(int argc, char const *argv[]) {
  ...
  while (1) {
    ...
    new_alphabet(file_read_buffer[0],sizeof(alphabet),alphabet);
